...
static func build_network_premise(premise: Premise, context: Dictionary) -> NetworkPremise:
	var is_variable := premise.operand_type == Premise.OperandType.VARIABLE
	...
	if is_variable:
		...
		network_codition.add_var_association(network_premise, premise.operand)
...
static func connect_network(rulebook: CompiledRulebook) -> void:
	...
	for rule in rulebook.rules:
		...
		for predicate in rule.condition.predicates:
			...
			for premise in predicate.premises:
				if premise is SimplePremise:
					...
					if previous_conjunction == null:
						connect_first_simple_premise(premise, conjunction)
					else:
		...
		var_processing.build_csp_constraint()


static func connect_first_simple_premise(premise: SimplePremise, conjunction: Conjunction) -> void:
...
static func connect_simple_premise(premise: SimplePremise, conjunction: Conjunction) -> void:
	conjunction.premise = premise
	premise.add.connect(conjunction.premise_add)
	premise.remove.connect(conjunction.premise_remove)
